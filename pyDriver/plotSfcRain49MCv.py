    ...
    for j in range(49):
        ...
        for i in a[0][b]:
            ...
            dn1d,dm1d,rrate1d,zkuc,zkasim,epst,piaku,piaka,dzdn,\
                ...
                piaka_sub,zetas,piakus,piakas = cmb.iter_profcv2(btop_,bzd_,bcf_,bsfc,zkul,zkal,dr,eps,\
                                                   imu,itype,dncv,dnp,piasrtku,relpiasrtku)
	    rrate,dmout = convretf90(dm_sub,rrate_sub,bzd,bcf,piaka_sub,piahb_sub,piasrtku,dpiasrt,relflagku,relflag,zetas)
            if bcf_>bbb+1:
                nc=bsfc_-bcf_
                wzku=0.2
                wzka=0.1
...
for i in np.array(indx[:]):#ind2[-20:])[[2,11,12,15,16,18]]:
    ...
    if 1==1:
        st_BB(pL,binBB,binBBTop,bcf,bsfc,btop,sfcType,pathAtten,relFlag,zm,zmka,f)
        st_noBB(pLnoBB,pType,binBB,binBBTop,bcf,bsfc,btop,bzd,sfcType,pathAtten,relFlag,zm,zmka,f)
        #stop
        ##
...
plt.suptitle('Mid-latitude Land')
plt.subplot(311)
...
plt.legend(['BB','noBB'])
plt.tight_layout()
plt.savefig('crossTrackStats_MidLandE.png')
