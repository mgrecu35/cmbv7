...
pL=[[] for i in range(25)]
pLnoBB=[[] for j in range(25)]
def st_BB(pL,binBB,binBBTop,bcf,bsfc,btop,pathAtten,relFlag,zm,zmka):
    for j in range(12,37):
        ...
        for i in a[0]:
            ...
            if bcf_>bbb+1:
                ...
                pL[j-12].append([rrate1d[bbb],rrate1d[bbt],rrate1d[bcf_],sfcRain[i,j]])


def st_noBB(pL,pType,binBB,binBBTop,bcf,bsfc,btop,bzd,pathAtten,relFlag,zm,zmka):
    for j in range(12,37):
        a=np.nonzero(binBB[:,j]<=0)
        b=np.nonzero(pType[:,j][a]==1)
        c=np.nonzero(pType[:,j][a][b]>0)
...
for i in np.array(indx[:]):#ind2[-20:])[[2,11,12,15,16,18]]:
    ...
    if 1==1:
        st_BB(pL,binBB,binBBTop,bcf,bsfc,btop,pathAtten,relFlag,zm,zmka)
        st_noBB(pLnoBB,pType,binBB,binBBTop,bcf,bsfc,btop,bzd,pathAtten,relFlag,zm,zmka)
        #stop
        ##
...
pL_s=[np.array(pL[i]).sum(axis=0) for i in range(25)]
pL_m=[np.array(pL[i]).mean(axis=0) for i in range(25)]
pLnoBB_s=[np.array(pLnoBB[i]).sum(axis=0) for i in range(25)]
pLnoBB_m=[np.array(pLnoBB[i]).mean(axis=0) for i in range(25)]
